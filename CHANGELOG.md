...
Reporting and celebration release. Nothing here touches share validation, difficulty, routing or payouts; every defect is in what the pool *says about itself*, which is why all of them survived so long. Two made a smart-port coin look idle on `/api/pools` while it was the active rotation slot and genuinely earning, and six left Avalon LEDs showing plain white through a block celebration that was sending correct RGB the entire time. Stratum, dashboard and script changes only: **no database migrations, no config format changes**. Drop-in upgrade from v2.7.0.

The LED fixes were applied in place on the v2.7.0 tag before this release was cut, so an operator who pulled that tag recently already has them.
...
### Known issues
- The dashboard's **manual** test-celebration endpoint still uses the old on/off path, so a celebration triggered from the UI button will still show white after this release. Pointing it at `block-celebrate.sh --test` is a separate change.
- Sentinel's auto-restart cooldown is recorded only when a restart succeeds, so a miner that cannot be restarted remotely is retried every monitor cycle indefinitely rather than every `auto_restart_cooldown` seconds. Unfixed here; the workaround is to disable auto-restart for affected devices.
...
- The LED changes carry no automated coverage. The deadline, lock-ownership and restore-skip logic were unit-tested during development, but the effect sequences themselves were never exercised against hardware and no Avalon is available in CI.
