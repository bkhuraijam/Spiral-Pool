...
The `block-celebrate.sh` LED fixes were applied in place on the v2.7.0 tag before this release was cut, so an operator who pulled that tag recently already has them. The test-endpoint, Sentinel cooldown and ZMQ-alerting fixes landed afterwards, in place on this tag.
...
- The LED changes carry no automated coverage. The deadline, lock-ownership and restore-skip logic were unit-tested during development, and the test endpoint's argument building was checked against a stubbed `Popen` — but the effect sequences themselves were never exercised against hardware and no Avalon is available in CI.
