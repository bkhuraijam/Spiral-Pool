...
Reporting and celebration release. Nothing here touches share validation, difficulty, routing or payouts; every defect is in what the pool *says about itself*, which is why all of them survived so long. Two made a smart-port coin look idle on `/api/pools` while it was the active rotation slot and genuinely earning, seven left Avalon LEDs showing plain white through a block celebration that was sending correct RGB the entire time, and one had Sentinel retry an impossible miner restart on every monitor cycle for as long as the miner stayed offline. Stratum, Sentinel, dashboard and script changes only: **no database migrations, no config format changes**. Drop-in upgrade from v2.7.0.

The `block-celebrate.sh` LED fixes were applied in place on the v2.7.0 tag before this release was cut, so an operator who pulled that tag recently already has them. The test-endpoint and Sentinel cooldown fixes landed afterwards, in place on this tag.
