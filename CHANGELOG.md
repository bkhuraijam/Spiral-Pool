...
- **A celebration that died with its host left the LED lit indefinitely, and there was no way to ask for dark-when-idle.** The restore path only ever put back the pre-celebration state, and it runs from `cleanup()` on the `EXIT` trap — which a power loss or SIGKILL never reaches. A real instance of this: a pool host went down mid-celebration and its miners held `LEDUser[3-100-90-0-255-255]` until manually cleared, that being exactly what phase 22 (`fast_pulse 0 255 255`) writes. Restoring the previous state is also of little value in practice, since the previous state is usually just whatever the last celebration left. A new `led_idle_state` key in the Sentinel config, read by the script the same way quiet hours already are, now decides: `"off"` (the default) puts the LED into mode 0 when the celebration ends, `"restore"` keeps the old behaviour. Both the normal end-of-celebration path and the signal handler go through one `restore_idle_led` helper, so they can no longer disagree. Note the two are not symmetrical on purpose: with `"restore"` a state that was never captured is left untouched rather than guessed — guessing is what the v2.7.1 `"Default cyan"` fallback did, whose leading `0` was mode OFF rather than a colour — whereas with `"off"` there is nothing to know and the LED simply goes dark. **This changes behaviour on upgrade**: LEDs that used to return to their prior state now go off. (`scripts/linux/block-celebrate.sh`, `src/sentinel/SpiralSentinel.py`.)
...
- Seven added in `tests/test_celebration_led.sh`, covering both celebration fixes: which states `restore_idle_led` writes under each `led_idle_state`, and that `is_cgminer` identifies an Avalon Nano3s whose plain-text `version` returns `Code=14`. That last one was verified to fail against the pre-fix probe. The script guards `main()` behind a `BASH_SOURCE` check, so the test sources it and calls the functions directly with the CGMiner API stubbed.
